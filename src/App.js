...
// Page routes shared between the top-level (no IDs in URL) and the nested
// BaseLayout (with IDs). Defined once so both stay in sync.
// Using relative paths so the same JSX works as both absolute (top-level)
// and relative-to-parent (inside BaseLayout) routes.
      ...
      <RouteHandler>
        <div className="App">
          <Routes>
            ...
            {/* Top-level page routes (no IDs in URL yet).
                Lets navigation from landing/home work before IDs are present;
                pages fall back to sessionStorage for IDs. */}
            {renderPageRoutes()}
